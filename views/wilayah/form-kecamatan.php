...
$pageTitle = $isEditMode ? 'Edit Informasi Kecamatan' : 'Input Kecamatan Sektor';
$saveLabel = $isEditMode ? 'Simpan Perubahan' : 'Registrasi Kecamatan';
$idKecamatan = (int) ($kecamatan['id'] ?? 0);

$selectedProvinsiId = $_GET['provinsi_id'] ?? null;
$selectedKabupatenId = $_GET['kabupaten_id'] ?? null;

if ($selectedProvinsiId === null && isset($kecamatan['kabupaten']['id_provinsi'])) {
  $selectedProvinsiId = $kecamatan['kabupaten']['id_provinsi'];
}
if ($selectedProvinsiId === null && isset($kecamatan['kabupaten']['id_parent'])) {
  $selectedProvinsiId = $kecamatan['kabupaten']['id_parent'];
}
if ($selectedKabupatenId === null && isset($kecamatan['id_kabupaten'])) {
  $selectedKabupatenId = $kecamatan['id_kabupaten'];
}
if ($selectedKabupatenId === null && isset($kecamatan['id_parent'])) {
  $selectedKabupatenId = $kecamatan['id_parent'];
}
?>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-sm text-slate-500">Ikat penamaan rayon kecamatan ke simpul kabupaten yang terotorisasi.</p>
            </div>
          </div>
        </div>

        <div class="rounded-2xl bg-white border border-slate-200 shadow-card overflow-hidden">
          <form action="index.php?controller=Wilayah&action=<?php echo $isEditMode ? ('updateKecamatan&id=' . $idKecamatan) : 'storeKecamatan'; ?>" method="POST" class="p-6 md:p-8">
            ...
            <div class="space-y-6">
              ...
              <div class="pt-4 border-t border-slate-100">
                <label for="nama" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nama Rayon Kecamatan <span class="text-red-500">*</span></label>
                <div class="relative">
                  <i class="fa-solid fa-draw-polygon absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                  <input
                    type="text"
                    class="w-full rounded-xl border border-slate-300 bg-slate-50 py-3 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none transition-all focus:border-emerald-500 focus:bg-white"
                    id="nama"
                    name="nama"
                    value="<?php echo htmlspecialchars($kecamatan['nama'] ?? $kecamatan['name'] ?? ''); ?>"
                    placeholder="Contoh: Kecamatan Tegalsari, Kec. Cidadap"
                    required
                  >
                </div>
                <p class="text-xs text-slate-400 mt-1.5 font-medium">Jangan lupakan prefix 'Kec.' atau 'Kecamatan'.</p>
              </div>

            </div>
            
            <div class="mt-8 pt-5 border-t border-slate-100 flex items-center justify-end gap-3">
              <a href="index.php?controller=Wilayah&action=indexKecamatan<?php echo $selectedKabupatenId ? ('&kabupaten_id=' . urlencode((string)$selectedKabupatenId)) : ''; ?>" class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors">
                Kembali
              </a>
              <button type="submit" class="px-6 py-2.5 rounded-xl bg-emerald-600 text-sm font-bold text-white hover:bg-emerald-700 hover:shadow-float transition-all shadow-sm">
...
<script>
  (function () {
    ...
    async function loadKabupatenByProvinsi(provinsiId) {
      kabupatenSelect.innerHTML = '<option value="">Memuat network...</option>';
      kabupatenSelect.disabled = true;

      try {
        const res = await fetch('index.php?controller=Wilayah&action=getKabupatenByProvinsi&id=' + encodeURIComponent(provinsiId), { credentials: 'same-origin' });
        const data = await res.json();
        kabupatenSelect.innerHTML = '<option value="">-- Pilih Kab target --</option>';
        ...
        kabupatenSelect.innerHTML = '<option value="">-- Gagal sinkronisasi --</option>';
      } finally {
    ...
    provinsiSelect.addEventListener('change', function () {
      if (this.value) {
        loadKabupatenByProvinsi(this.value);
      } else {
        kabupatenSelect.innerHTML = '<option value="">-- Pilih Kab target --</option>';
      }
    });
